<div id="litbaskets-libraries" style="display: none; width: 90%; max-width: 90rem; margin: 0 auto; padding: 1rem;">
	<h1>Open Source Libraries</h1>
	<p>
		Litbaskets is an open source project licensed under the MIT license.
		Check out the GitHub repository:
		<a href="https://github.com/blairw/litbaskets">https://github.com/blairw/litbaskets</a>.
	</p>
	<p>
		The following sets forth attribution notices for third party software that may be contained in portions of the Litbaskets product.
		We thank the open source community for all of their contributions.
	</p>

	<h2>Bootstrap</h2>
	<pre><?php include ('./node_modules/bootstrap/LICENSE'); ?></pre>

	<h2>bootstrap-switch</h2>
	<pre><?php include ('./node_modules/bootstrap-switch/LICENSE'); ?></pre>

	<h2>clipboard</h2>
	<pre>MIT License http://zenorocha.mit-license.org/ © Zeno Rocha</pre>

	<h2>jQuery</h2>
	<pre><?php include ('./node_modules/jquery/LICENSE.txt'); ?></pre>

	<h2>Patternfly</h2>
	<pre><?php include ('./node_modules/patternfly/LICENSE.txt'); ?></pre>

	<h2>Underscore</h2>
	<pre><?php include ('./node_modules/underscore/LICENSE'); ?></pre>
</div>